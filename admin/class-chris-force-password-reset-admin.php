 ...
 * @subpackage fpr_Chris_Force_Password_Reset/admin
 */
 ...
 */
class fpr_Chris_Force_Password_Reset_Admin {
	...
	public function fpr_add_action_links( $links ) {
		...
		*/
		$settings_page_link = 'fpr-settings-page-slug';
	   $settings_link = array(
		'<a href="' . admin_url( 'options-general.php?page=fpr-settings-page-slug').'">' . __('Settings', $this->plugin_name) . '</a>',
		);
	...
	public function fpr_profile_update( $user_id ) {
		...
		if( 
			...
			(1 == $editor  		 && current_user_can('editor')) ||
			...
			(1 == $subscriber	 && current_user_can('subscriber'))
				...
				if(isset($_POST['pass1']) && '' != $_POST['pass1'] && strlen($_POST['pass1']) > 1 ){
					...
					//update the options table
					 update_option( $current_logged_in_user_id , $value);
				}
		}
	}
	...
	public function fpr_password_reset_called( $user, $new_pass ) {
		...
		$user_id = $user->ID;
		$current_logged_in_user_id = $user_id."_password_update_timestamp";
	 ...
	 */ 
 	public function fpr_settings_page() {
				...
				add_options_page(
								...
								'fpr-settings-page-slug',  //Page slug
	...
	public function fpr_validate_password_reset() {
		...
		if(
			...
			//if you havent updated your password after the plugin was installed
			if(	'' == $last_password_update_time  &&
				(
					(1 == $administrator && current_user_can('administrator')) ||
					(1 == $editor  		 && current_user_can('editor')) ||
					(1 == $author  		 && current_user_can('author')) || 
